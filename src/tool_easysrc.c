...
static bool easysrc_performed;

/* Set once a transfer has been performed, so that the next generated
   statement resets the handle first: the tool uses a fresh easy handle per
   operation, the generated code reuses one. Deferred rather than emitted
   right after curl_easy_perform() so that no reset is generated when nothing
   follows. */
static bool easysrc_pending_reset = FALSE;
...
CURLcode easysrc_add(struct slist_wc **plist, const char *line)
{
  ...
  struct slist_wc *list;
  if(easysrc_pending_reset && (plist == &easysrc_code)) {
    /* cleared first: the recursive call adds to easysrc_code as well */
    easysrc_pending_reset = FALSE;
    result = easysrc_add(&easysrc_code, "curl_easy_reset(curl);");
    if(result)
      return result;
  }
  list = slist_wc_append(*plist, line);
  if(!list) {
    easysrc_free();
...
{
  easysrc_performed = FALSE;
  return easysrc_add(&easysrc_code, "curl = curl_easy_init();");
}

bool easysrc_is_first(void)
{
  return !easysrc_performed;
}
...
CURLcode easysrc_perform(void)
{
  ...
  if(!result)
    result = easysrc_add(&easysrc_code, "");
  if(!result)
    easysrc_performed = TRUE;

  /* Options set for this operation must not leak into the next one */
  if(!result)
    easysrc_pending_reset = TRUE;
...
{
  CURLcode result;
  /* nothing follows, so no reset is needed before the cleanup */
  easysrc_pending_reset = FALSE;
  result = easysrc_add(&easysrc_code, "curl_easy_cleanup(curl);");
  if(!result)
