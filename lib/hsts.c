...
static CURLcode hsts_out(struct stsentry *sts, FILE *fp)
{
  ...
  if(sts->expires != TIME_T_MAX) {
    ...
    if(!result) {
      ...
      if(curl_mfprintf(fp, "%s%s \"%d%02d%02d %02d:%02d:%02d\"\n",
                    sts->includeSubDomains ? "." : "", sts->host,
                    stamp.tm_year + 1900, stamp.tm_mon + 1, stamp.tm_mday,
                    stamp.tm_hour, stamp.tm_min, stamp.tm_sec) < 0)
        return CURLE_WRITE_ERROR;
    }
  }
  else if(curl_mfprintf(fp, "%s%s \"%s\"\n",
                  sts->includeSubDomains ? "." : "", sts->host,
                  UNLIMITED) < 0)
    return CURLE_WRITE_ERROR;
  return CURLE_OK;
                        ...
                        const char *file)
{
  ...
  if(!result) {
    if(fputs("# Your HSTS cache. https://curl.se/docs/hsts.html\n"
             "# This file was generated by libcurl! Edit at your own risk.\n",
             out) == EOF)
      result = CURLE_WRITE_ERROR;
    for(e = Curl_llist_head(&h->list); e && !result; e = n) {
      struct stsentry *sts = Curl_node_elem(e);
      n = Curl_node_next(e);
      result = hsts_out(sts, out);
    }
    if(curlx_fclose(out))
      result = result ? result : CURLE_WRITE_ERROR;
