                                   ...
                                   struct url_conn_match *m)
{
  ...
  else if(get_protocol_family(m->needle->scheme) & PROTO_FAMILY_SMTP) {
    if(!Curl_smtp_conns_match(m->needle, conn))
      return FALSE;
  }
#endif
#ifndef CURL_DISABLE_RTSP
  else if(get_protocol_family(m->needle->scheme) & PROTO_FAMILY_RTSP) {
    if(!Curl_rtsp_conns_match(m->data, m->needle, conn))
      return FALSE;
  }
#endif
