...
static size_t t3233_read(char *buffer, size_t size, size_t nitems,
                         void *instream)
{
  (void)buffer;
  (void)size;
...
static void t3233_stop(CURL *easy)
{
...
static CURLcode test_unit3233(const char *arg)
{
  ...
  fail_unless(curl_mime_data_cb(part, 10, t3233_read, NULL, NULL, NULL) ==
  ...
  fail_unless(curl_mime_data_cb(part,
                                 ...
                                 t3233_read, NULL, NULL, NULL) == CURLE_OK,
              "curl_mime_data_cb()");
  sz = multipart_size(mime);
  ...
  part = curl_mime_addpart(mime);
  abort_unless(part, "curl_mime_addpart()");
  fail_unless(curl_mime_data_cb(part, CURL_OFF_T_MAX / 2, t3233_read,
