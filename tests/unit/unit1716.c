...
static CURLcode t1716_setup(void)
{
...
static void t1716_create_key(struct t1716_key *key,
                             const char *hostname,
                             uint16_t port,
                             uint8_t type)
{
...
static CURLcode test_unit1716(const char *arg)
{
...
#define IPV6ONLY(x) &skip
  ...
  for(i = 0; i < CURL_ARRAYSIZE(tests); ++i) {
    ...
    t1716_create_key(&entry_id, tests[i].host, tests[i].port, CURL_DNST_ADDR);
     ...
     miss. */
  {
    ...
    t1716_create_key(&entry_id, "stale.example", 80, CURL_DNST_ADDR);
    dns = Curl_hash_pick(&multi->dnscache.entries,
                         entry_id.data, entry_id.len);
    if(dns)
