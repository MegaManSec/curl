...
static const unsigned char unit3307_random[32] = {
...
static const unsigned char unit3307_secret[16] = {
  0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff, 0x11, 0x22,
  0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0x00
};

static CURL_THREAD_RETURN_T CURL_STDCALL unit3307_open_thread(void *arg)
{
  int i;
  (void)arg;
...
static CURL_THREAD_RETURN_T CURL_STDCALL unit3307_write_thread(void *arg)
{
  int i;
  (void)arg;
  for(i = 0; i < NUM_ITERATIONS; i++) {
    Curl_tls_keylog_write("CLIENT_RANDOM", unit3307_random,
                          sizeof(unit3307_random),
                          unit3307_secret, sizeof(unit3307_secret));
...
static CURL_THREAD_RETURN_T CURL_STDCALL unit3307_close_thread(void *arg)
{
  int i;
  (void)arg;
  for(i = 0; i < NUM_ITERATIONS; i++) {
...
static void unit3307_setenv(const char *value)
{
#ifdef _WIN32
...
static void unit3307_unsetenv(void)
{
#ifdef _WIN32
...
static CURLcode test_unit3307(const char *arg)
{
  ...
  for(i = 0; i < NUM_OPEN_THREADS; i++)
    open_th[i] = Curl_thread_create(unit3307_open_thread, NULL);
  for(i = 0; i < NUM_WRITE_THREADS; i++)
    write_th[i] = Curl_thread_create(unit3307_write_thread, NULL);
  for(i = 0; i < NUM_CLOSE_THREADS; i++)
    close_th[i] = Curl_thread_create(unit3307_close_thread, NULL);
