...
/* This exercises the nonce-verification loop that Curl_wakeup_init() uses
   ...
   immediately closes the connection must make the handshake fail promptly,
   not spin forever, since a clean EOF is not the same as EAGAIN. */

static CURLcode test_unit1962(const char *arg)
{
  ...
  /* the "attacker": connects and is about to be accepted */
  ...
  /* the peer connects and immediately closes, well before it would have
     echoed the nonce back */
  sclose(peer);
  ...
  fail_unless(rc != 0, "verify_nonce must fail on a peer EOF");
  fail_if(elapsed_ms > 5000,
          "verify_nonce took too long to detect the peer EOF");
