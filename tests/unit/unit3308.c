...
static CURLM *unit3308_multi;
static int unit3308_calls;
static CURLMcode unit3308_remove_mresult;

static CURLcode unit3308_export_remove(CURL *handle, void *userptr,
                                       ...
                                       size_t earlydata_max)
{
  ...
  unit3308_calls++;
  unit3308_remove_mresult = curl_multi_remove_handle(unit3308_multi, handle);
  return CURLE_OK;
}

static CURLcode unit3308_export_detach(CURL *handle, void *userptr,
                                       const char *session_key,
                                       const unsigned char *shmac,
                                       size_t shmac_len,
                                       const unsigned char *sdata,
...
static CURLcode unit3308_add_ticket(struct Curl_easy *data)
{
  struct Curl_ssl_session *s = NULL;
  struct dynbuf packed;
  CURLcode result;
  void *sdata_copy = curlx_strdup("unit3308-ticket");

  if(!sdata_copy)
    return CURLE_OUT_OF_MEMORY;

  result = Curl_ssl_session_create(sdata_copy, strlen("unit3308-ticket"),
  ...
  result = Curl_ssl_session_import(data, "unit3308.example:443:G", NULL, 0,
...
static CURLcode test_unit3308(const char *arg)
{
  ...
  result = unit3308_add_ticket(easy1);
  ...
  unit3308_multi = multi;
  unit3308_calls = 0;
  unit3308_remove_mresult = CURLM_OK;
  result = Curl_ssl_session_export(easy1, unit3308_export_remove, NULL);
  fail_unless(result == CURLE_OK, "export failed");
  fail_unless(unit3308_calls == 1, "export callback should run once");
  fail_unless(unit3308_remove_mresult == CURLM_RECURSIVE_API_CALL,
  ...
  unit3308_calls = 0;
  result = Curl_ssl_session_export(easy1, unit3308_export_detach, NULL);
  fail_unless(result == CURLE_OK, "export failed");
  fail_unless(unit3308_calls == 1, "export callback should run once");
  fail_unless(!Curl_ssl_scache_is_locked_by_current_thread(easy2),
