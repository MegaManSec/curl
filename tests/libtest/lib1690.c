...
#define NTFY1690_PRIME_ITERATIONS 125
...
static size_t discard1690_cb(char *ptr, size_t size, size_t nmemb,
                             void *userp)
{
  (void)ptr;
  (void)userp;
...
static void notify1690_cb(CURLM *multi, unsigned int notification,
                          CURL *easy, void *notifyp)
{
  struct ntfy1690_ctx *ctx = notifyp;
  (void)multi;
  ...
  if(ctx->count == 1) {
    ...
    if(ctx->c) {
      ...
      curl_easy_setopt(ctx->c, CURLOPT_WRITEFUNCTION, discard1690_cb);
...
static CURLcode test_lib1690(const char *URL)
{
  CURLM *multi = NULL;
  struct ntfy1690_ctx ctx;
  int i;
  int running = 1;
  int numfds;
  int loops;
  ...
  multi_setopt(multi, CURLMOPT_NOTIFYFUNCTION, notify1690_cb);
  ...
  easy_setopt(ctx.a, CURLOPT_WRITEFUNCTION, discard1690_cb);
  multi_add_handle(multi, ctx.a);

  easy_init(ctx.b);
  easy_setopt(ctx.b, CURLOPT_URL, URL);
  easy_setopt(ctx.b, CURLOPT_WRITEFUNCTION, discard1690_cb);
  multi_add_handle(multi, ctx.b);

  for(i = 0; i < NTFY1690_PRIME_ITERATIONS; i++) {
