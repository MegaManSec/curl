...
static CURLcode t3597_ssl_ctx_cb(CURL *curl, void *sslctx, void *clientp)
{
  struct t3597_rec *rec = clientp;
  (void)curl;
...
static CURL *t3597_handle(const char *url, const char *cafile,
                          struct t3597_rec *rec)
{
  CURL *curl = NULL;
  ...
  easy_setopt(curl, CURLOPT_SSL_CTX_FUNCTION, t3597_ssl_ctx_cb);
...
static CURLcode t3597_run(CURLM *multi, CURL *curl)
{
  CURLcode result = CURLE_OK;
  int still_running = 0;
...
static CURLcode test_lib3597(const char *URL)
{
  CURL *curl1 = NULL, *curl2 = NULL;
  CURLM *multi = NULL;
  struct t3597_rec rec1, rec2;
  CURLcode result = CURLE_OK;
  ...
  curl1 = t3597_handle(URL, libtest_arg2, &rec1);
  if(!curl1) {
  ...
  result = t3597_run(multi, curl1);
  if(result)
    goto test_cleanup;

  curl2 = t3597_handle(URL, libtest_arg2, &rec2);
  if(!curl2) {
    result = TEST_ERR_EASY_INIT;
    goto test_cleanup;
  }

  result = t3597_run(multi, curl2);
  if(result)
    goto test_cleanup;
