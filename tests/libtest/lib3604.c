 ...
 * cacheable CAfile criteria, do not end up with the same X509_STORE when
 * both set CURLOPT_SSL_CTX_FUNCTION. Each such handle must get its own
...
static CURLcode t3604_ssl_ctx_cb(CURL *curl, void *sslctx, void *clientp)
{
  struct t3604_rec *rec = clientp;
  (void)curl;
  rec->store = SSL_CTX_get_cert_store((SSL_CTX *)sslctx);
  rec->called = 1;
  return CURLE_OK;
}

static CURL *t3604_handle(const char *url, const char *cafile,
                          struct t3604_rec *rec)
{
  CURL *curl = NULL;
  ...
  easy_setopt(curl, CURLOPT_SSL_CTX_FUNCTION, t3604_ssl_ctx_cb);
...
static CURLcode t3604_run(CURLM *multi, CURL *curl)
{
  CURLcode result = CURLE_OK;
  int still_running = 0;
...
static CURLcode test_lib3604(const char *URL)
{
  CURL *curl1 = NULL, *curl2 = NULL;
  CURLM *multi = NULL;
  struct t3604_rec rec1, rec2;
  CURLcode result = CURLE_OK;
  ...
  if(curl_global_sslset(CURLSSLBACKEND_OPENSSL, NULL, NULL) != CURLSSLSET_OK) {
    curl_mfprintf(stderr, "could not set OpenSSL as backend\n");
  ...
  curl1 = t3604_handle(URL, libtest_arg2, &rec1);
  if(!curl1) {
  ...
  /* Run the first transfer to completion before starting the second, so
     ...
     X509_STORE cannot get freed and replaced by an unrelated one at the
     same address before the comparison below. */
  result = t3604_run(multi, curl1);
  if(result)
    goto test_cleanup;

  curl2 = t3604_handle(URL, libtest_arg2, &rec2);
  if(!curl2) {
    result = TEST_ERR_EASY_INIT;
    goto test_cleanup;
  }

  result = t3604_run(multi, curl2);
  if(result)
    goto test_cleanup;
  ...
  if(rec1.store == rec2.store) {
    curl_mfprintf(stderr, "FAIL: both handles share the same "
                  "X509_STORE while both set CURLOPT_SSL_CTX_FUNCTION\n");
    result = TEST_ERR_FAILURE;
    goto test_cleanup;
  }
