...
#define T1688_MAX_13_LIFETIME_SEC (60 * 60 * 24 * 7)
#define T1688_MAX_12_LIFETIME_SEC (60 * 60 * 24)
#define T1688_SLACK_SEC 60
...
struct t1688_ctx {
  struct t1688_found found[4];
  size_t count;
};

static size_t t1688_make_ticket(uint8_t *p, int ietf_id, curl_off_t until)
{
  size_t i;
...
static CURLcode t1688_export_cb(CURL *easy, void *userptr,
...
static const struct t1688_found *t1688_find(struct t1688_ctx *ctx,
                                            const char *key)
{
  size_t i;
...
static CURLcode test_lib1688(const char *URL)
{
  ...
  struct t1688_ctx ctx;
  const struct t1688_found *f;
  ...
  t1688_make_ticket(ticket, 0x0304, now + 365 * 86400);
  ...
  t1688_make_ticket(ticket, 0x0303, now + 365 * 86400);
  ...
  t1688_make_ticket(ticket, 0x0304, now - 3600);
  ...
  result = curl_easy_ssls_export(easy, t1688_export_cb, &ctx);
  if(result) {
  ...
  f = t1688_find(&ctx, "expired.example.test:443:h2:G");
  if(f) {
    curl_mfprintf(stderr, "expired ticket was not discarded on import\n");
    goto test_cleanup;
  }

  f = t1688_find(&ctx, "tls13.example.test:443:h2:G");
  if(!f) {
    curl_mfprintf(stderr, "tls13 ticket missing from export\n");
    goto test_cleanup;
  }
  if(f->valid_until > now + T1688_MAX_13_LIFETIME_SEC + T1688_SLACK_SEC) {
  ...
  f = t1688_find(&ctx, "tls12.example.test:443:h2:G");
  if(!f) {
    curl_mfprintf(stderr, "tls12 ticket missing from export\n");
    goto test_cleanup;
  }
  if(f->valid_until > now + T1688_MAX_12_LIFETIME_SEC + T1688_SLACK_SEC) {
